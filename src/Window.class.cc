...
void Window::createFramebuffers ()
{
	swapChainFramebuffers.resize ( swapChainImageViews.size (),
	                               VkDeleter<VkFramebuffer>{ device, vkDestroyFramebuffer } );
...
VkResult Window::CreateDebugReportCallbackEXT ( VkInstance instance,
...
void Window::initWindow ()
{
	...
	window = glfwCreateWindow ( this->xSize, this->ySize, "Vulkan", nullptr, nullptr );
...
void Window::createTextureImage ()
{
	...
	stbi_uc *pixels = stbi_load ( "assets/tex/test.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha );
	...
	VkDeleter<VkImage> stagingImage{ device, vkDestroyImage };
	VkDeleter<VkDeviceMemory> stagingImageMemory{ device, vkFreeMemory };
...
void Window::createImageView ( VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, VkDeleter<VkImageView> &imageView )
{
...
void Window::createImage ( uint32_t width,
                           uint32_t height,
                           VkFormat format,
...
void Window::copyImage ( VkImage srcImage, VkImage dstImage, uint32_t width, uint32_t height )
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands ();
...
void Window::createBuffer ( VkDeviceSize size,
...
void Window::createShaderModule ( const std::vector<char> &code, VkDeleter<VkShaderModule> &shaderModule )
{
...
VkExtent2D Window::chooseSwapExtent ( const VkSurfaceCapabilitiesKHR &capabilities )
{
	...
	{
		VkExtent2D actualExtent = { this->xSize, this->ySize };
...
void Window::DestroyDebugReportCallbackEXT ( VkInstance instance,
...
VKAPI_ATTR VkBool32 VKAPI_CALL Window::debugCallback ( VkDebugReportFlagsEXT flags,
